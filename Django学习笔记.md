...
* 时间格式化：`val1|date:"Y-m-d"`，可以**按照指定字符串格式化传入的datetime对象参数**，注意y m d前没有百分号![](pictures/date过滤器格式.png)
...
4. 至此，在所有模板中，无需传递自定义上下文，就可以访问`frontuser` 
...
* User 模型类：django框架自带的User模型，包含常见的用户名、密码、邮箱等字段

  * 创建普通用户、超级用户

  * 验证用户， `authenticate()` 

  * 字段扩展：继承原有User类的，重写manager、User类；settings.py中重新设置

  * 与用户的相关函数： `authenticate()` 、`get_user_mdoel()` 获取当前项目的User

* 权限：针对一个app的权限，增删改查；为模型增加属性 
...
django的授权系统包括 验证 和 授权 两个部分，验证用于校验用户身份，授权用于校验用户权限。

创建完一个django项目后，授权系统就已经默认继承。包括
...
* 中间件：
  ...
  * AuthenticationMiddleware ：用来处理和当前 session 相关联的用户。
...
![](pictures/redis与memcached比较.png)
...
* 连接：`redis-cli -h [ip] -p [端口]` ，下述所有命令均在连接上redis后操作

  redis默认只允许本机连接，即`127.0.0.1`，如果希望其他机器远程访问本机redis，需要进入redis配置文件，修改`bind 127.0.0.1 [your_ip]`，后面添加自己的ip，则其他机器就可以用指定ip访问。

* 添加：`sety key "value" [EX  timeout]`。key存在时会被覆盖。value不添加双引号时，默认类型就是字符串型，但字符串不能有空格，否则会被认为多个值，可以用双引号包裹解决这个问题。EX timeout表示设置键值对的超时时间，默认永不过期。
...
* Serializer
  ...
  * 创建 ：
    * app下创建`serializers.py`，字段声明，create()、update()

  * 序列化
    * 序列化实例： `serializer(instance)` ，返回`serializer.data`
      * 表单验证：`Serializer(data=request.POST)`, is_valid ,save 

* ModelSerializer：自动捕获字段，声明方式更简单 ；模型+字段
* 模型存在外键关联时候的嵌套
...
* 1对多：例如Goods与Merchant。Goods被序列化时，返回Merchant的id、序列化时返回关联模型的id、创建时提交id。Merchant在被关联时会创建字段`goods_set`，但是该字段不会被`ModelSerializer`序列化，需要显示指定。
...
1. get：取数据 ，如`merchant/`、`merchant/31`
...
* Request 
  * POST 表单数据; data，任意数据
* Response
  * 状态码的添加 关键字`Response(data,status=)`
* APIView 类视图，简单，和模型、序列化类脱钩，手动实现各种method
* Mixins 类视图，挂钩模型、序列化类，插拔式继承，实现了method的底层方法，按需实现method
* Generic 类视图，挂钩模型、序列化类，插拔式继承，直接实现了method，增加了列表数据的获取
* 分页：设置类视图的属性 `pagination_class = MerchantPagination`，同时定义对应的分页类
...
如果是视图函数，那么可以使用装饰器`rest_framework.decorators.api_view`进行装饰，这个装饰器中可以传递本视图函数可以使用什么`method`进行请求。示例代码如下：
...
* ViewSet ：实现方法，路由注册 
* ModelViewSet：挂钩模型、序列化类，无需拔插继承，直接实现method方法
* 搜索类视图：增加类视图属性` filter_backends=[]`
...
class MerchantViewSet(viewsets.ViewSet):
    ...
    最基础的ViewSet 就像 APIView 一样要自己实现CURD逻辑 
    '''
...
* 认证
  ...
  * 认证以后会对request增加用户信息，例如user属性等

  * BasicAuthentication 验证流程：拆解请求头的`Authorization:xx`，得到`user:pwd`键值对，验证

  * JSON Web Token认证机制：
    * 验证流程：（1）第一次携带用户名、密码 （2）后台验证，生成token，交给前端（3）第二次及以后，前台携带token返回（3）后台验证token
      * token包含的信息：用户名、密码、过期时间
      * 生成、解码token：encode、decode
      * 配置认证：在settings中设置；在视图中设置

* 权限：

  * 权限思想：限制不同用户的method，从而实现增删改查
  * 配置权限 
...
1. 从`request.META`中取出`{Authorization:'basic xxx}'`键值对，META存储的是请求头HEADERs的信息 。如果没有该键值对，则直接返回None。
...
![image-20210530183754390](../better/resources/image-20210530183754390.png)
...
![img](../better/resources/MultipleProxySetup.png)
...
6. 前端项目执行`npm run build`打包，项目目录下生成`dist`文件夹，结构如下：
...
<img src="https://gitee.com/jiang_hui_kai/images/raw/master/img/image-20220123140027241.png" alt="image-20220123140027241" style="zoom:67%;" />
 ...
 7. 将步骤6中的除了`index.html`的文件拷贝到django项目的`static`文件夹中，作为静态资源访问。配置settings.py 文件 
    ...
    ![image-20210626153919324](https://gitee.com/jiang_hui_kai/images/raw/master/img/image-20210626153919324.png)

8. 将 index.html 文件拷贝到 templates 文件夹下，**django只渲染它，其余的跳转由vue组件转换后的js脚本来完成**。将index.html 的资源依赖改成 static 链接 （css前面加入了static）

   ```html
    <link href="/static/css/app.62f2d439.css" rel="preload" as="style">
   ```

9.  添加关于 index.html 的路由，将其渲染并返回 
...
django作为服务器app，在真正的生产环境时，应该代之以更强的app，uwsgi的作用正在于此。

### WSGI协议

WSGI：**Python Web Server Gateway Interface**，缩写为WSGI，是为python语言定义的web服务器（以下简称server）与web应用（以下简称app）之间的通用接口，没有官方实现，**是一种协议**。
